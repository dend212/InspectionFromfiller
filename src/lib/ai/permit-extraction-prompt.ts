...
/**
 * Stamped onto `inspection_records.extraction_version` with every `done`
 * extraction. Bump it (date.serial) whenever the system prompt, the output
 * schema or the wire coercion changes in a way that should invalidate stored
 * facts: D7 reuse only replays facts read under the current version and
 * re-reads the rest (the stored PDF is kept, so no re-download).
 */
export const PERMIT_EXTRACTION_VERSION = "2026-09-14.2";
...
- documentKind: from the title of the FIRST page you were given, except on a later-pages pass (the instructions say the first pages did not settle the permit identity): there, if ANY page you were given is an Approval / Authorization to Construct, a Discharge Authorization or a Final DA — including a county approval stamp on a site plan or drawing — report that kind, even when the first page of the attachment is an inspection card, invoice or plan sheet, and report the county approval / issue date on that page as issueDate. A DA packet that also contains an older Approval to Construct is a discharge_authorization.
...
- waterSource: one of municipal, private_company, shared_well, private_well, hauled_water. Never emit the form's own label (do not write "water_company", "city" or "well"). On a Discharge Authorization "Water Company" is the county's category for any piped utility, so decide municipal vs private_company from the provider named in "Water Source ID #". A city, town or county utility (City of X Water, Town of Y, or a "city" / "city water" box) → municipal. A place name that merely contains "City" (Sun City, Sun City West) is not a city utility — decide by the provider actually named. Any other named water company or utility (EPCOR, Arizona Water Company, Liberty Utilities, Global Water, "… Water Co.", "… Water Users Association", "… Domestic Water Improvement District"), or a "water company" box beside a separate "city" box → private_company. "Shared Well" checked, or a well named in "Shared Well:" → shared_well. "Private Well", "Domestic Well", "Exempt Well", "Individual Well" or a 55-xxxxxx well registration → private_well. "Hauled" → hauled_water. When the page says only "Water Company" and names no provider, leave waterSource out and say so in notes. Quote the line naming the provider as evidence.
