...
function makeRequest(body?: unknown): Request {
  return new Request("http://localhost/api/inspections/insp-1/return", {
    method: "POST",
...
describe("POST /api/inspections/[id]/return", () => {
  ...
  it("returns to draft status with a note", async () => {
    const res = await POST(makeRequest({ note: "Fix the address" }), makeParams("insp-1"));
    expect(res.status).toBe(200);
    expect((await res.json()).status).toBe("draft");
  });

  it("returns to draft status without a note", async () => {
    const res = await POST(makeRequest({}), makeParams("insp-1"));
    expect(res.status).toBe(200);
  });

  it("returns 409 when inspection is not in_review", async () => {
    mockDbUpdate.mockResolvedValueOnce([]); // atomic guard fails
    const res = await POST(makeRequest({ note: "Fix" }), makeParams("insp-1"));
