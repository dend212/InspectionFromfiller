...
- **No persisted-schema change for the authority fix itself:** `ProvenanceEntry` and `provenance-schema.ts` unchanged; the new `authority` lives as an optional SIBLING of `provenance` on `ProposedField` (`merge.ts:87` spreads only `provenance`; the PATCH schema strips unknown provenance keys, so never put it inside provenance). **The 2026-09-14 extraction-version stamp DOES need one:** `0016_inspection_records_extraction_version.sql` adds `inspection_records.extraction_version` (nullable, no backfill). Apply it to the remote BEFORE merging/pushing the branch (`node --env-file=.env.local scripts/apply-extraction-version-migration.mjs`, which verifies via `information_schema.columns`) — `run-store` selects every schema column, so the app hard-fails on prefill until the column exists. Migrations are hand-applied here (the drizzle journal only knows 0000), so nothing orders this automatically.
- **Dedupe order (exact):** `SOURCE_RANK` (permit 3 > assessor 2 > listing 1 > scan 0) → lower `authority.docRank` wins, a proposal without `authority` counts as rank 0 → strictly higher confidence → first-seen. No document-date tie-break (it would flip the phase-2 `recordsAvailable` proposal — see `with-extraction.ts:93`).
  - **Superseded 2026-09-14 (11420 N Saint Andrews Way):** the owner's precedence rule is now "newest Discharge Authorization on top, then the Approval to Construct, then Notices of Transfer and abandonments". `DOC_CLASS_RANK` gained a `discharge_auth` class and re-scaled to `{ discharge_auth: 0, permit: 1, permit_sub: 2, notice_of_transfer: 3, abandonment: 4, plan_review: 5, other: 6 }` (`FINAL DA` / `DA FINAL` / `DA` / `*DISCHARGE*` classify as `discharge_auth`; `permitDocRank` lives in `permits/doc-types.ts` and maps `discharge_authorization` / `final_da` → 0, `approval_to_construct` → 1). A document-date tie-break DOES exist now, between two record proposals that BOTH carry `authority`: `authority.docDate` (the extracted issue date for DA / ATC kinds, the EDMS docDate for transfer / abandonment kinds — never the EDMS docDate for a permit-class document, since in the legacy archive it is the scan date) — newer wins, dated beats undated, then confidence, then first-seen. The phase-2 index row carries no `authority`, so it is still never displaced by date and keeps its first-seen tie. The Dove Valley fixture's ranks read 1 (PERMIT) and 3 (NOT) under the new scale.
...
- [ ] From the repo root: `rm -rf .next && npm run build` → passes. `npx vitest run --reporter=dot` → the ONLY failures are the pre-existing 12 (rbac/roles/nav/mobile-nav, download route ×2, reopen route ×2, constants STEP_LABELS, validators ×2 …) — anything else is this branch's regression: fix with a focused commit and re-run. `npx tsc --noEmit -p tsconfig.json 2>&1 | grep -F -f <(git diff --name-only main...HEAD)` style check: no NEW type errors in files this branch touched. (Use `-F -f <(...)`, one touched-file path per pattern line — NOT a hand-built `-E` alternation of unescaped paths: a Next.js `[id]`-style dynamic-route segment is parsed by `-E` as a bracket character class rather than a literal substring, so that file's alternative silently stops matching its own path.)
