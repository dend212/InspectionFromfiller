...
## A11. Extraction sends a flat wire schema; `PermitFactsSchema` stays the domain contract (Phase 3 Task 5 / 11)

Live smoke 2026-09-12: `messages.parse()` with `zodOutputFormat(PermitFactsSchema)` is rejected by the API before any tokens are billed — `Schemas contains too many parameters with union types (18 parameters with type arrays or anyOf … limit: 16)`; deduplicating the 18 nullable fact objects through `$ref` (8 unions) and even a 12-fact nested variant then fail with `The compiled grammar is too large`. Only a flat list of uniform rows compiles.
