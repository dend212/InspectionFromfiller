...
Provider v1 (amendment A10): Apify actor `api-ninja/zillow-property-details-scraper` (plain-address input; ~$0.015/lookup, pay-per-event) called with `POST https://api.apify.com/v2/acts/api-ninja~zillow-property-details-scraper/run-sync-get-dataset-items?token=$APIFY_TOKEN&timeout=60&memory=1024` and body `{ "property": ["8956 E Venus Dr, Carefree, AZ 85377"] }` (never send `maxTotalChargeUsd` — Apify rejects it for pay-per-event actors). Water source and sewer are read from the MLS-sourced `resoFacts.waterSource` / `resoFacts.sewer` arrays of ARMLS strings (take `[0]`); `bedrooms`, `bathrooms`, `yearBuilt`, `lotSize` (sqft, else `lotAreaValue` × 43 560 when `lotAreaUnits` is Acres) and `hdpUrl` (path, prefixed with `https://www.zillow.com`; `zpid` fallback) are top-level. Utilities are only present for active listings (`homeStatus` FOR_SALE/PENDING) — an off-market parcel returns `null` for both and yields a listing with no water/sewer facts. The mapper stays defensive (case-insensitive deep key search, `resoFacts` first, single-element arrays unwrapped) and any unmapped shape yields `null` facts, never an error. Rejected: `sian.agency/zillow-property-detail-scraper` (never returns utilities); alternative if this one degrades: `axesso_data/zillow-property-details-scraper-v2` (URL/ZPID input).

Water-source normalisation (case-insensitive, ARMLS vocabulary per A10): `City Water` / `city|municipal|public` → `municipal`; `Pvt Water Company` / `Private Water Company` / `Water Co` → `private_company`; `Well - Shared` / `Shared Well` → `shared_well`; `Well - Pvtly Owned` / `Private Well` / bare `Well` → `private_well`; `Hauled` → `hauled_water`; `None`/unknown → no fact. Sewer: any `septic` (`Septic Tank`, `Septic in & Cnctd`) → `septic`; any `sewer` (`Sewer - Public`, `Sewer - Private`, `Public Sewer`) → `sewer`; `None`/other → `unknown` (not evidence either way, no warning). Listing proposals carry `confidence 0.8` (`waterSource`) / `0.85` (`bedrooms`) — listing data can be stale, but above the fill gate. A listing that says `sewer` proposes nothing but adds a **warning** proposal on `facilityInfo.wastewaterSource` rendered as an amber chip: `Listing says "Sewer" — confirm this property is on septic`.
